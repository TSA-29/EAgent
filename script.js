...
function displayExam(data) {
    ...
    data.questions.forEach(q => {
        ...
        html += `<div class="question-title">Question ${q.id}</div>`;
        html += `<div class="question-text">${q.question}</div>`;
        html += '<div class="options">';

        ['A', 'B', 'C', 'D'].forEach(opt => {
            html += `<div class="option" onclick="selectAnswer(${q.id}, '${opt}')">`;
            html += `<input type="radio" name="q${q.id}" id="q${q.id}${opt}" value="${opt}">`;
            html += `<label for="q${q.id}${opt}">${opt}. ${q.options[opt]}</label>`;
            html += '</div>';
        });
...
function displayResults(correct, total) {
    ...
    examData.questions.forEach(q => {
        ...
        html += `<div>Your answer: ${userAnswers[q.id]} ${isCorrect ? '✓' : '✗'}</div>`;
        if (!isCorrect) {
            html += `<div class="correct-answer">Correct answer: ${q.answer}</div>`;
        }
        html += '</div></div>';
    });
